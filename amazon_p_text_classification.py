...
def main():
    ...
    fin = io.open('./dataset/crawl-300d-2M.vec', 'r', encoding='utf-8', newline='\n', errors='ignore')
    ...
    with open("./amazon/train.ft.txt", encoding='utf-8') as f1:
        while True:
            line = f1.readline()
            if not line:
                break
            train_label.append(int(line[9]) - 1)
            train_text.append(line[11:])

    test_text = []
    test_label = []

    with open("./amazon/test.ft.txt", encoding='utf-8') as f2:
        while True:
            line = f2.readline()
            if not line:
                break
            test_label.append(int(line[9]) - 1)
            test_text.append(line[11:])
